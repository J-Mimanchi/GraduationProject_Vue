<template>
      ...
      <el-form :model="data.form" ref="formRef" label-width="70px" style="padding: 20px">
        <div style="text-align: center; margin-bottom: 20px">
          <el-upload class="avatar-uploader" :action="fileUploadUrl" :show-file-list="false"
                     :headers="{ myshoptoken: data.form.token }" :on-success="handleAvatarSuccess" >
        ...
        <div style="text-align: center">
          <el-button type="primary" @click="save" style="background-color: #c518ff; border-color: #c518ff;">保 存</el-button>
        </div>
      </el-form>
    </div>
  </template>
  ...
  <script setup>
    ...
    const data = reactive({
      form: JSON.parse(localStorage.getItem('myshop-user') || '{}'),
    })
    // 头像上传成功
    ...
    const save = () => {
      request.put('/user/update', data.form).then(res => {
        if (res.code === '200') {
          ...
          // 更新缓存
          localStorage.setItem('myshop-user', JSON.stringify(data.form))
          emit('updateUser')
        } else {
